...
AgML contains a wide variety of public datasets from various locations across the world. See the
[dataset coverage map](/) on the homepage for a country-by-country breakdown.
...
- **Image Classification**: a `label` `ClassLabel` column.
- **Object Detection**: an `objects` column, holding COCO-style bounding boxes and category IDs per image.
- **Semantic Segmentation**: a `mask` column, a single-channel (`L`-mode) image the same size as its `image`.
